...
const Permissions: React.FC = () => {
  ...
  const canEditTargetRole = (targetRole: string) => {
    ...
    if (currentUserRole === 'manager' && (targetRole === 'admin' || targetRole === 'owner'))
      return false;
    if (currentUserRole === 'editor' || currentUserRole === 'viewer') return false;
    return true;
  };
  const isCheckboxDisabled = (targetRole: string, perm: string, currentValue: boolean): boolean => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 p-6 text-gray-100">
      <div className="mx-auto max-w-3xl">
        <h1 className="mb-6 text-center text-3xl font-bold">Adjust Workspace Permissions</h1>

        {Object.keys(matrix).length === 0 ? (
          <p className="text-center text-gray-400">Loading permissions...</p>
        ) : (
          Object.entries(matrix).map(([role, permissions]) => (
            <div key={role} className="mb-6 rounded-lg border border-gray-700 bg-gray-800/50 p-4">
              <h2 className="mb-3 text-xl font-semibold capitalize">{role} Permissions</h2>
