...
import { FiAlertTriangle, FiCheck, FiInbox, FiLoader, FiX } from 'react-icons/fi';
...
function Inbox() {
  ...
  const handleAcceptInvite = async (invite: IInbox) => {
    ...
    try {
      await api.post('/inbox/user/invite/accept', { invite });
      setInbox((prev) =>
        prev.map((i) => (i._id === invite._id ? { ...i, response: 'accepted' } : i))
      );
    } catch (error) {
      console.log(error);
  ...
  const handleDeclineInvite = async (invite: IInbox) => {
    setProcessingId(invite._id);
    try {
      await api.post('/inbox/user/invite/decline', { invite });
      // Update local state immediately
      setInbox((prev) =>
        prev.map((i) => (i._id === invite._id ? { ...i, response: 'declined' } : i))
      );
    } catch (error) {
      console.log(error);
  ...
  return (
    <div className="min-h-screen bg-neutral-900 p-4 pt-8 text-gray-300 md:p-10">
      <h1 className="font-display mb-8 text-5xl font-bold text-white">Your Inbox</h1>
      ...
      {!loading && inbox.length === 0 ? (
        <div className="flex h-64 flex-col items-center justify-center rounded-2xl border-4 border-dashed border-neutral-700 p-16 text-center">
          ...
          <span className="block text-2xl font-semibold text-white">Your inbox is empty</span>
          <p className="mt-2 text-gray-400">Workspace invitations will appear here.</p>
        </div>
      ) : (
        // --- List of Invites ---
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3">
                    <div className="space-y-2 p-6 md:col-span-2">
                      ...
                      <p className="text-gray-400">{invite.payload.workspaceDescription}</p>
                    ...
                    <div className="flex items-center justify-center rounded-bl-2xl bg-neutral-900/50 p-6 md:rounded-r-2xl md:rounded-bl-none">
                      <div>
                        <p className="text-sm font-semibold text-indigo-400 uppercase">
                          Your Requested Role
                        </p>
                        <p className="font-display text-4xl font-bold text-white capitalize">
                          {invite.payload.role}
                        </p>
                      </div>
                    </div>
                      ...
                      <div className="flex items-center gap-2 text-lg font-medium">
                        ...
                        <span
                          className={`font-bold capitalize ${
                            invite.response === 'accepted' ? 'text-green-400' : 'text-red-400'
                          }`}
                        >
                          {invite.response}
                        </span>
